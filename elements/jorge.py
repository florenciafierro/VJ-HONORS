...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, pressed_keys):
        ...
        if self.invulnerable and pygame.time.get_ticks()>self.invulnerable_hasta:
            self.invulnerable=False
    ...
    def shoot(self, mouse_pos): 
        ...
        disparo_triple =tiempo_actual<=self.disparo_triple_hasta
        if not disparo_triple:
            projectile=Projectile(self.rect.center,direction,self.screen_width,self.screen_height)
            self.projectiles.add(projectile)
        else:
            angulo=math.atan2(direction[0],direction[1])
            d=math.radians(10)
            for a in (angulo-d,angulo,angulo+d):
                componente_x=math.cos(a)
                componente_y=math.sin(a)
                projectile=Projectile(self.rect.center,(componente_x,componente_y),self.screen_width,self.screen_height)
                self.projectiles.add(projectile)
