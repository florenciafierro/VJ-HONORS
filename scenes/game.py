...
def gameLoop():
    ...
    mensaje_velocidad=""
    velocidad_tiempo=0
    mensaje_disparo_triple=""
    disparo_triple_tiempo=0
    ...
    while running:
        ...
        if puntaje>=50 and not hasattr(player,"triple_recibido"):
            player.disparo_triple_hasta=pygame.time.get_ticks()+10000
            mensaje_disparo_triple="Dispara triple por 10 segundos!"
            disparo_triple_tiempo=pygame.time.get_ticks()+3000
            player.triple_recibido=True
        ...
        mira.update()
        if player.invulnerable and pygame.time.get_ticks()>player.invulnerable_hasta:
